...
let parentKey, grandparentKey, isAlbum, isArtist;
...
const plexamp_nfc = (nfc) => {
	...
	nfc.on('reader', (reader) => {
		...
		const es = new EventSource(`${getPlayerUri()}/:/eventsource/notifications`);
					...
					case 'playing':
						if (data.parentKey && data.grandparentKey) {
							...
							if (
								...
								!decoded.includes(`${data.parentKey}/`)
							) {
								console.log(
							...
							if (
								...
								!decoded.includes(`${data.grandparentTitle}/`)
							) {
		...
		reader.on('card', async (card) => {
			...
			try {
				...
				if (
					...
					if (isActiveCard(cardUri) && resumeOnCardPlacement) {
						await resume_player();
					} else {
						uri = cardUri;
						...
						if (trackCardState) {
							const setCardState = function (e) {
								const data = JSON.parse(e.data);
								switch (data.state) {
									case 'playing':
										if (data.parentKey && data.grandparentKey) {
											es.removeEventListener('state', setCardState, false);
											const decoded = decodeURIComponent(uri?.search);
											isAlbum = decoded.includes(`${data.parentKey}/`);
											if (isAlbum) {
												console.log(
													`Album tag placed: ${data.grandparentTitle} - ${data.parentTitle}`
												);
											}
											isArtist = decoded.includes(`${data.grandparentKey}/`);
											if (isArtist) {
							...
							es.addEventListener('state', setCardState, false);
						}
					}
				} else {
					console.log(
		...
		reader.on('card.off', async (card) => {
			...
			try {
				...
				if (cardUri && isActiveCard(cardUri) && pauseOnCardRemoval) {
					await pause_player();
				}
			} catch (err) {
				console.error(err.toString());
...
const isActiveCard = (testUri) => {
	const decoded = decodeURIComponent(testUri?.search);
	if (trackCardState && isAlbum) {
		return testUri?.href === uri?.href && decoded.includes(`${parentKey}/`);
	} else if (trackCardState && isArtist) {
		return testUri?.href === uri?.href && decoded.includes(`${grandparentKey}/`);
	} else {
		return testUri?.href === uri?.href;
	}
};
